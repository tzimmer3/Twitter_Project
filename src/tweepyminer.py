...
"""
    def get_friend_list(self, num_friends):
        friend_list = []
        for friend in Cursor(self.twitter_client.friends, id=self.twitter_user, wait_on_rate_limit=True, wait_on_rate_limit_notify=True).items(num_friends):
            friend_list.append(friend)
        return friend_list

    def get_home_timeline_tweets(self, num_tweets):
        home_timeline_tweets = []
        for tweet in Cursor(self.twitter_client.home_timeline, id=self.twitter_user, wait_on_rate_limit=True, wait_on_rate_limit_notify=True).items(num_tweets):
